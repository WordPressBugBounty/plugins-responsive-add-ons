<?php
/**
 * Site Builder — read-only display rules matcher.
 *
 * Backward-compat port of Responsive Pro's
 * class-responsivepro-site-builder-display-rules.php, trimmed to only the
 * methods needed to *match* which layout(s) apply to the current front-end
 * request. Nothing here writes data or powers the editor UI.
 *
...
if ( ! class_exists( 'RASB_Legacy_Display_Rules' ) ) {

	class RASB_Legacy_Display_Rules {
		...
		/**
		 * @var string|null current page type
		 */
		private static $current_page_type = null;

		/**
		 * @var array current page data
		 */
		...
		/**
		 * Get posts matching the display-location conditions for the current request.
		 *
		 * @param string $post_type Post type (resp-site-builder).
		 * @param array  $option    Meta keys: location / exclusion / users / enabled.
		 * @return array
		 */
