<?php
/**
 * Backward-compat port of ResponsiveX Pro's megamenu nav walker.
 * Read-only: renders megamenu postmeta already saved on nav menu items.
 * Does not expose any settings UI or save endpoint.
 *
...
if ( ! class_exists( 'Responsive_Legacy_MegaMenu_Nav_Walker' ) ) {

	/**
	 * Responsive_Legacy_MegaMenu_Nav_Walker description
	 */
	class Responsive_Legacy_MegaMenu_Nav_Walker extends Walker_Nav_Menu {
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( 0 === $depth ) {
				...
				// Only trust the enable flag if it's backed by the signed marker
				// Pro writes when it genuinely saves megamenu settings. Without
				// this, anyone could flip the megamenu on for free by adding the
				// plain `enable_megamenu` custom field by hand.
				if ( ! $this->verify_legacy_marker( $item->ID ) ) {
					$this->megamenu = '';
				}
		...
		/**
		 * Verifies the signed marker ResponsiveX writes whenever it
		 * genuinely saves megamenu settings for this menu item. Uses the
		 * site's own AUTH_KEY/AUTH_SALT (via wp_hash()) so it can't be
		 * reproduced by anyone who only has access to add a custom field
		 * through the UI or an import — they'd need code execution on the
		 * server to read those secrets, at which point this check is no
		 * longer the weakest link.
		 *
		 * @param int $item_id Menu item post ID.
		 * @return bool
		 */
		private function verify_legacy_marker( $item_id ) {
			$stored   = get_post_meta( $item_id, $this->prefix . 'legacy_marker', true );
			if ( empty( $stored ) ) {
				return false;
			}
			$expected = wp_hash( 'responsive_megamenu_legacy|' . $item_id, 'auth' );
